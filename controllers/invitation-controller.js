...
const invitation = asyncWrapper(async (req, res) => {
  const { inviter, family, inviteeEmail, firstName, inviteeInviteLater } = req.body;

  if (!inviteeInviteLater) {
    const duplicate = await invitationService.findInviteeDuplicate(inviteeEmail, family);
    ...
    await emailService.sendInvitationEmail(inviteeEmail, family, emailVerificationToken, firstName);
    ...
    if (!duplicate) {
      ...
      await invitationService.createInvitation(inviter, family, inviteeEmail, invitationURL);
